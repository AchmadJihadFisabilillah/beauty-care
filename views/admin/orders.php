...
<form method="get" class="toolbar">
    ...
    <select name="payment">
        ...
        <?php foreach (['pending','waiting_verification','paid','rejected'] as $p): ?>
            <option value="<?= $p ?>" <?= $payment === $p ? 'selected' : '' ?>>
    ...
    <tbody>
        <?php foreach ($orders as $order): ?>
            <tr>
                ...
                <td>
                    <a href="<?= BASE_URL ?>/admin/order-detail?id=<?= $order['id'] ?>">
                        Detail
                    </a>
                </td>
            </tr>
...
<div class="pagination">
    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a 
            ...
            href="<?= BASE_URL ?>/admin/orders?status=<?= urlencode($status) ?>&payment=<?= urlencode($payment) ?>&page=<?= $i ?>"
        >
            <?= $i ?>
        </a>
    <?php endfor; ?>
